#![allow(unused)] // while exploring for prod
...
async fn main() -> Result<()> {
    ...
    // --- Delete 
    let sql = "DELETE $th";
    let vars = [ ("th".into(), thing(&t1)?.into())].into();
    ds.execute(sql, ses, Some(vars), true).await?;
...
async fn create_task((ds, ses): &DB , title: &str , priority: i32) -> Result<String> {
    ...
    into_iter_objects(ress)?
        .next()
        .transpose()?
        .and_then(|obj| obj.get("id").map(|s| s.to_string()))
    ...
    match res {
        Some(Value::Array(arr)) => {
            let it = arr.into_iter().map(|v| match v {
                ...
                _ => Err(anyhow!("a record is not an object")),
            });
            Ok(it)
        }
